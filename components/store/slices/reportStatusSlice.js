...
const tgsInitialState = {
  ...
  // systemControllersStatus: {
  //   instantHeat: { isActivated: 4, fault: 1 },
  //   snowSensor: { isReady: 3, isActivated: 0, fault: 1 },
  //   windFactor: { isReady: 2, isActivated: 0, fault: 1 },
  //   heatingSchedule: { isReady: 0, isActivated: 1, fault: 1 },
  //   constantTemp: { isReady: 3, isActivated: 0, fault: 1 },
  // },
...
const reportStatusSlice = createSlice({
  ...
  reducers: {
    handleReportStatus: (state, action) => {
      const { essSwitch, tgsSwitch, tesSwitch, isF } = action.payload;
...
const getMachines = (switchData, location, machineType, isF) => {
  ...
  const systemConsumption = () => {
    ...
    // Object.keys(switchData).forEach((machine) => {
    //   total += +calculateTotalEnergyConsumption(switchData[machine].reading, machineType, isF) || 0;
    // });
  ...
  // return Object.keys(switchData).map((machine) => {
  //   const machineData = switchData[machine];
  //   const consumption = calculateTotalEnergyConsumption(machineData.reading, machineType, isF)
  //   return {
  //     [machine]: {
  //       ...initialState,
...
export const selectReportStatus = (state) => state.reportStatus;
export const {
  handleReportStatus
